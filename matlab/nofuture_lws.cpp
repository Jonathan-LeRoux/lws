...
void mexFunction(int nlhs, mxArray *plhs[], /* Output variables */
    ...
    if (nrhs < 3){
        mexPrintf("nofuture_lws: not enought inputs\n");
        return;
    }      
    ...
    if (!IS_2D_FULL_DOUBLE(S_IN)) {
        mexPrintf("nofuture_lws: spectrogram must be full 2-D double matrix.\n");
        return;
    }
    #ifdef DEBUG
    mexPrintf("nofuture_lws: S_in is size %d x %d\n", mxGetM(S_IN), mxGetN(S_IN));
    #endif
    ...
    if (K!= 3){
        mexPrintf("nofuture_lws: weights should be 3-dimensional.\n");
    }
    int L = Wdims[0]-1;
    int Q = Wdims[1];
    #ifdef DEBUG
    mexPrintf("nofuture_lws: weights is size %d x %d x %d\n", Wdims[0],Wdims[1],Wdims[2]);
    #endif
    ...
    if (nlhs > 0) {
        ...
        for (int j=0; j<iterations; j++){
            ...
            if (Q==2){
                NoFuture_LWSQ2(ExtSr, ExtSi, pWr, pWi, pWflag, AmpSpec, Nreal, T, L, threshold);
            }
            else if (Q==4){
                NoFuture_LWSQ4(ExtSr, ExtSi, pWr, pWi, pWflag, AmpSpec, Nreal, T, L, threshold);
            }else {
                NoFuture_LWSanyQ(ExtSr, ExtSi, pWr, pWi, pWflag, AmpSpec, Nreal, T, L, Q, threshold);
            }
        }
